...
## Обновление результатов прогнозов

После синхронизации завершённых матчей можно рассчитать исходы Telegram-прогнозов:

```powershell
python -m jobs.update_results
```

Команда проверяет pending-прогнозы с привязанным матчем и рынком `match_winner`, выставляет `win`, `loss` или `void` по результату матча и печатает summary в консоль. ROI каналов считается по уже рассчитанным прогнозам, поэтому перед просмотром рейтинга каналов полезно обновить результаты.

## Поддерживаемые рынки прогнозов

- `match_winner` - победа команды в матче.
- `maps_total` - тотал карт: `ТБ 2.5 карт`, `ТМ 2.5 карт`, `over 2.5 maps`, `under 2.5 maps`, `больше 2.5 карт`, `меньше 2.5 карт`.

Для `maps_total` результат считается по сумме карт `team_a_score + team_b_score`: over выигрывает, если сумма больше line, under выигрывает, если сумма меньше line, равенство line считается `void`.
